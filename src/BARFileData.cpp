...
void CGDACSequenceResultItem::GetData(int iData, int iCol, BarSequenceResultData &data )
{
	...
	else
	{
		...
		{
			ReadInt(m_lpData + offset, data.iValue);
		}
		else
		{
			ReadFloat(m_lpData + offset, data.fValue);
		}
	}
}
...
void CBARFileData::SetFileName(const char *name)
{
...
bool CBARFileData::ReadHeaderSection()
{
	...
	// Number of sequendes
	ReadInt(instr, m_NumberSequences);


	// Columns
	int i = 0;
	int cType;
	ReadInt(instr, m_NumberColumns);
	...
	// Parameter
	std::string str;
	ReadInt(instr, m_NumberParameters);
	m_Parameters.resize(m_NumberParameters);
	TagValuePairType param;
	for (i=0; i<m_NumberParameters; i++)
	{
		ReadString(instr, str);
...
bool CBARFileData::ReadDataSection()
{
	...
	{
		ReadString(instr, m_Results[i].m_Name);
		if (bVersion2)
		{
			ReadString(instr, m_Results[i].m_GroupName);
		}
		ReadString(instr, m_Results[i].m_Version);
		if (bVersion2)
		{
			int nParams=0;
			ReadInt(instr, nParams);
			for (int iParam=0; iParam<nParams; iParam++)
			{
				ReadString(instr, tag);
				ReadString(instr, val);
				m_Results[i].AddParameter(tag, val);
			}
		}
		ReadInt(instr, m_Results[i].m_NumberDataPoints);
		...
		for (int j=0; j<m_Results[i].m_NumberDataPoints; j++)
		{
			...
			for (int k=0; k<m_NumberColumns; k++)
			{
				if (m_ColumnTypes[k] == BAR_DATA_INTEGER)
					ReadInt(instr, m_Results[i].m_ppData[j][k].iValue);
				else
					ReadFloat(instr, m_Results[i].m_ppData[j][k].fValue);
			}
		}
